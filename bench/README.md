...
The **Step-0 measurement tool** for the ZAYA1-on-RDNA3 performance work
(`/home/pat/.claude/plans/to-align-with-the-cryptic-fairy.md`). It measures the
metric we are optimizing — **RSA query latency** — plus answer accuracy (the
regression gate for accuracy-affecting changes) and, when Prometheus is
reachable, server-side peak concurrency and decode throughput.
...
(`running ≈ 16, waiting ≈ 0`) instead of queueing.
